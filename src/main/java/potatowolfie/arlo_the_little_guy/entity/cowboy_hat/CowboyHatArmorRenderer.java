...
public class CowboyHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("arlo-the-little-guy", "textures/models/armor/cowboy_hat.png");
    ...
    public void render(PoseStack matrices, SubmitNodeCollector queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new CowboyHatModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(ModEntityModelLayers.COWBOY_HAT)
            );
        }
