...
public class BowlerHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.of("arlo-the-little-guy", "textures/models/armor/bowler_hat.png");
    ...
    public void render(MatrixStack matrices, OrderedRenderCommandQueue queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new BowlerHatModel(
                    MinecraftClient.getInstance().getLoadedEntityModels().getModelPart(ModEntityModelLayers.BOWLER_HAT)
            );
        }
