...
public class SunHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("arlo-the-little-guy", "textures/models/armor/sun_hat.png");
    ...
    public void render(PoseStack matrices, SubmitNodeCollector queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new SunHatModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(ModEntityModelLayers.SUN_HAT)
            );
        }
