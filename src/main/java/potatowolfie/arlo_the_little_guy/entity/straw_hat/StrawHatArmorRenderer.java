...
public class StrawHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("arlo-the-little-guy", "textures/models/armor/straw_hat.png");
    ...
    public void render(PoseStack matrices, SubmitNodeCollector queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new StrawHatModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(ModEntityModelLayers.STRAW_HAT)
            );
        }
