...
public class TopHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("arlo-the-little-guy", "textures/models/armor/top_hat.png");
    private TopHatModel model;
    ...
    public void render(PoseStack matrices, SubmitNodeCollector queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new TopHatModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(ModEntityModelLayers.TOP_HAT)
            );
        }
