...
public class TricornHatArmorRenderer implements ArmorRenderer {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("arlo-the-little-guy", "textures/models/armor/tricorn.png");
    private TricornHatModel model;
    ...
    public void render(PoseStack matrices, SubmitNodeCollector queue, ItemStack stack,
        ...
        if (this.model == null) {
            this.model = new TricornHatModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(ModEntityModelLayers.TRICORN)
            );
        }
